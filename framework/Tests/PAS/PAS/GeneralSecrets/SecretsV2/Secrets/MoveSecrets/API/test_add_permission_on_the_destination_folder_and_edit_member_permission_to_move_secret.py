...
@pytest.mark.pasapi
@pytest.mark.bhavna
def test_add_permission_on_the_destination_folder_and_edit_member_permission_to_move_secret(
        core_session,
        users_and_roles,
    ...
    """
    test method to move the secret with add & view permission of destination folder for User A &
    edit member permissions of source secret for User A
    ...
    assert result_move['success'], f'Not Able to move the secret into destination Folder: {result_move["Result"]}'
    logger.info(f'Able to move the secret into destination folder with "ADD" permissions:{result_move["Message"]}')
