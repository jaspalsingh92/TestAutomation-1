...
def test_add_system_with_unmanaged_account(core_session, setup_pas_system_for_unix):
    """
    Test case : C279341
    ...
    assert system_rows['Name'] == system_info[0], f"failed to find system {system_info[0]} in portal "
    logger.info(f"system found {system_rows}")

    account, status = ResourceManager.get_accounts_from_resource(core_session, system_id)
    assert status, f'failed to find {account} in response, status returned is {status}'
