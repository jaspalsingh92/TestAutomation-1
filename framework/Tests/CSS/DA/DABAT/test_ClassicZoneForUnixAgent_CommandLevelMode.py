# test_ClassicZoneForUnixAgent_CommandLevelMode.py
...
def test_4_3_command_level_mode_1(
...
@pytest.mark.css
@pytest.mark.da_unix_bat
def test_command_level_mode_login_with_ad_user_anu0007_to_run_audited_command(
        log_test_name, dc_is_joined, set_da_installation, disable_da_nss, cda_login_as_aduser_0007, login_as_root,
        cda_login_am_as_domain_admin):
    """
    Login with AD user to run audited command
    """
    logger.info("--- Case C1238997")
    ...
    cda_login_as_aduser_0007.send_shell_command("adinfo")
...
@pytest.mark.css
@pytest.mark.da_unix_bat
def test_4_3_command_level_mode_5(
        log_test_name, dc_is_joined, login_as_root):
    """
    ...
    login_as_root.send_command("dacontrol -e -c /usr/share/centrifydc/libexec/adinfo")
